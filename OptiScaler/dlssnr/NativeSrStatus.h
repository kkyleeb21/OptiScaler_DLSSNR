...
struct Status {
    ...
    bool succeeded=false, rayReconstruction=false;
    bool Live(uint64_t now) const {return succeeded && lastTick && now>=lastTick && now-lastTick<=1500;}
};
inline std::mutex mutex;
inline Status status;
inline void Record(const void* handle,bool success,uint64_t tick,bool rr=false) {
    std::lock_guard lock(mutex);
    status.rayReconstruction=rr;status.handle=handle;status.succeeded=success;status.lastTick=tick;
