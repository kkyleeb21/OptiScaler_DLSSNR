...
static std::unordered_map<const NVSDK_NGX_Handle*, NativeOnlyFeature> nativeOnlyHandles;
    ...
    *OutHandle = nullptr;
    ...
    if (NativeOnlyEnabled() && (InFeatureID == NVSDK_NGX_Feature_SuperSampling ||
        (InFeatureID == NVSDK_NGX_Feature_RayReconstruction &&
         DlssNr::ReProfile::NativeRr(state.gameExe.c_str()))))
    {
        if (!InParameters) return NVSDK_NGX_Result_FAIL_InvalidParameter;
        *OutHandle = nullptr;
        ...
        if (!cfg.DLSSEnabled.value_or_default() || !NVNGXProxy::IsDx12Inited() || !create)
            return NVSDK_NGX_Result_FAIL_FeatureNotSupported;
        const char* nativeKind = InFeatureID == NVSDK_NGX_Feature_RayReconstruction ? "RR" : "SS";
        ...
        LOG_INFO("RE native: native {} CreateFeature result=0x{:X}, handle={}",
            nativeKind, (uint32_t)result, (void*)*OutHandle);
...
NVSDK_NGX_API NVSDK_NGX_Result NVSDK_NGX_D3D12_ReleaseFeature(NVSDK_NGX_Handle* InHandle)
{
    ...
    if (nativeOnly) {
        ...
        LOG_INFO("RE native: entering native ReleaseFeature id={}", id);
        ...
        LOG_INFO("RE native: native ReleaseFeature id={} result=0x{:X}",id,(uint32_t)result);
                                               ...
                                               PFN_NVSDK_NGX_ProgressCallback InCallback)
{
    ...
    NVSDK_NGX_Feature nativeFeature = NVSDK_NGX_Feature_SuperSampling;
    { std::lock_guard lock(nativeOnlyMutex);
      auto it = nativeOnlyHandles.find(InFeatureHandle);
      if (it != nativeOnlyHandles.end()) {
          nativeOnly = true; nativeFrame = ++it->second.frames; nativeFeature = it->second.feature;
      } }
    if (nativeOnly)
    {
        if (!InParameters) return NVSDK_NGX_Result_FAIL_InvalidParameter;
        ...
        if (nativeFeature == NVSDK_NGX_Feature_RayReconstruction) {
            ...
            DlssNr::NativeSr::Record(InFeatureHandle,result==NVSDK_NGX_Result_Success,GetTickCount64(),true);
