...
function Start-D18GuiOperation {
    ...
    if(-not $Request.ContainsKey('cacheSession')){
        if(-not (Get-Variable -Name D18GuiCacheSession -Scope Script -ErrorAction SilentlyContinue)){$script:D18GuiCacheSession=[guid]::NewGuid().ToString('N')}
        $Request.cacheSession=$script:D18GuiCacheSession
    }
    ...
    $command = '$ProgressPreference="SilentlyContinue";$s=[Text.Encoding]::UTF8.GetString([Convert]::FromBase64String("' + $data + '")) | ConvertFrom-Json; & $s.worker -RequestPath $s.request -ResultPath $s.result; exit $LASTEXITCODE'
...
function Get-D18GuiRequest {
    ...
    if ($Action -notin @('Uninstall','CleanupCache')) {
