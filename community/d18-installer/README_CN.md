...
当前安装包版本为 `0.1.2`，统一压缩包名称为 `DLSSNR_D18_0.1.2.zip`。
...
| `UNVERIFIED_COMPATIBLE` | 此 Runtime 未经验证，但未发现与安装程序的补丁要求冲突，可能可用。备份后继续；运行异常时请先换已验证版本复现。部分字段已打补丁、其余字段符合要求时也属于此类。 |
| `CONFLICT` | 此 Runtime 的关键字段与安装程序冲突，无法安全应用补丁。原文件未修改，请更换已验证版本。提示包含首个冲突偏移与输入 SHA-256。 |
...
0.1.2 更新通用核心、UI 和诊断功能，不包含鬼武者专用兼容补丁。
...
[简短更新说明](RELEASE_NOTES_0.1.2.md) · [功能说明](COMMON_FEATURES.md)。
