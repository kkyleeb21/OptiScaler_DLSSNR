...
int main(){
 ...
 auto enable=(void(*)(int))GetProcAddress(m,"D24SetEnabled");auto release=(int(*)(void*))GetProcAddress(m,"D24Release");
 ...
 for(int f=0;f<3;++f){c->ClearUnorderedAccessViewFloat(ou.Get(),color);int r=run(&owner,c.Get(),&p,NVSDK_NGX_DLSS_Feature_Flags_MVLowRes|(test>=4?NVSDK_NGX_DLSS_Feature_Flags_IsHDR:0));printf("test=%u frame=%d result=%d\n",test,f,r);fflush(stdout);if(r!=1)return 10;
