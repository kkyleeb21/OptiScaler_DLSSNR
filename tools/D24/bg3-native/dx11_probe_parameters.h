...
#include <map>
#include <string>
#ifdef D24_PARAMETER_READ_AUDIT
#include <set>
#endif
class ProbeParameters final : public NVSDK_NGX_Parameter {
    std::map<std::string,double> numbers;
    std::map<std::string,void*> pointers;
    void observe(const char* n) const {
#ifdef D24_PARAMETER_READ_AUDIT
        if(readKeys.size()<256)readKeys.insert(n);
#else
        (void)n;
#endif
    }
    template<class T> NVSDK_NGX_Result number(const char* n,T* v) const {observe(n);auto i=numbers.find(n);if(i==numbers.end())return NVSDK_NGX_Result_FAIL_InvalidParameter;*v=static_cast<T>(i->second);return NVSDK_NGX_Result_Success;}
    template<class T> NVSDK_NGX_Result pointer(const char* n,T** v) const {observe(n);auto i=pointers.find(n);if(i==pointers.end())return NVSDK_NGX_Result_FAIL_InvalidParameter;*v=static_cast<T*>(i->second);return NVSDK_NGX_Result_Success;}
public:
#ifdef D24_PARAMETER_READ_AUDIT
