...
void highPass(ID3D12Device* dev,ID3D12RootSignature* root,ID3D12PipelineState* pso,
    ...
    const auto upload=[&](UINT w,UINT h,Pattern pattern,bool writable,bool half=false){
        auto td=texture(w,h,writable);if(half)td.Format=DXGI_FORMAT_R16G16B16A16_FLOAT;auto dst=create(dev,td,D3D12_HEAP_TYPE_DEFAULT,D3D12_RESOURCE_STATE_COPY_DEST);
        ...
        for(UINT y=0;y<h;++y)for(UINT x=0;x<w;++x){auto p=pattern(x,y);if(half){unsigned short packed[4];for(int i=0;i<4;++i)packed[i]=DirectX::PackedVector::XMConvertFloatToHalf(p[i]);memcpy(mem+y*fp.Footprint.RowPitch+x*8,packed,8);}else memcpy(mem+y*fp.Footprint.RowPitch+x*sizeof(Pixel),p.data(),sizeof(Pixel));}
    ...
    auto out=upload(c.Width,c.Height,base,true,true);auto keep=upload(c.Width,c.Height,base,true);
    ...
    for(auto* dst:{out.Get(),keep.Get()}){D3D12_UNORDERED_ACCESS_VIEW_DESC v{};v.Format=dst->GetDesc().Format;v.ViewDimension=D3D12_UAV_DIMENSION_TEXTURE2D;
    ...
    auto d=(c.Mode==0?keep.Get():out.Get())->GetDesc();D3D12_PLACED_SUBRESOURCE_FOOTPRINT fp{};UINT64 bytes;dev->GetCopyableFootprints(&d,0,1,0,&fp,nullptr,nullptr,&bytes);
    ...
    for(UINT y=0;y<c.Height;++y)for(UINT x=0;x<c.Width;++x){auto p=expected(x,y);float v[4];if(c.Mode==0)memcpy(v,mem+y*fp.Footprint.RowPitch+x*16,16);else for(int i=0;i<4;++i)v[i]=DirectX::PackedVector::XMConvertHalfToFloat(reinterpret_cast<unsigned short*>(mem+y*fp.Footprint.RowPitch+x*8)[i]);
        for(int ch=0;ch<4;++ch) if(!std::isfinite(v[ch]) || std::abs(v[ch]-p[ch])>0.0005f*std::max(1.f,std::abs(p[ch]))){std::printf("mode=%u at %u,%u c%d got %.8f expected %.8f\n",c.Mode,x,y,ch,v[ch],p[ch]);throw std::runtime_error("high-resolution pixels");}}
...
int wmain(int argc,wchar_t**)try{
    DlssNr::HighResolution::Size s{};
    require(DlssNr::HighResolution::Plan({0,0,3840,2160},1.25f,s)&&s.width==4800&&s.height==2704,"aligned plan");
    require(!DlssNr::HighResolution::Plan({0,0,0,2160},1.25f,s),"zero size");
    require(!DlssNr::HighResolution::Plan({0,0,16384,16384},1.5f,s),"dimension cap");
    require(!DlssNr::HighResolution::Plan({0,0,3840,2160},NAN,s),"NaN scale");
    require(!DlssNr::HighResolution::Budget(0,1024,0,{32,24}),"budget denial");
    ...
    D3D12_COMPUTE_PIPELINE_STATE_DESC pd{};pd.pRootSignature=root.Get();pd.CS={DlssNr_HighResolution_cso,sizeof(DlssNr_HighResolution_cso)};ComPtr<ID3D12PipelineState> pso;check(dev->CreateComputePipelineState(&pd,IID_PPV_ARGS(&pso)));
    auto p=[](UINT,UINT){return Pixel{.25f,.4f,.7f,1};};auto zero=[](UINT,UINT){return Pixel{0,0,0,0};};
    DlssNrConstants c{};c.RelativeColour=1;c.WhitePoint=1;c.MaxRatio=4;c.DebugScale=1;c.Passthrough=1;c.NetworkRatioX=c.NetworkRatioY=1;
    for(auto size:{DlssNr::HighResolution::Size{32,24},{48,32}}){
        c.Mode=5;c.Width=size.width;c.Height=size.height;c.ValidX=3;c.ValidY=2;c.ValidWidth=21;c.ValidHeight=17;
        highPass(dev.Get(),root.Get(),pso.Get(),c,28,23,p,p,zero,p);
        c.Mode=6;c.Width=28;c.Height=23;
        auto inside=[&](UINT x,UINT y){return x>=c.ValidX&&y>=c.ValidY&&x-c.ValidX<c.ValidWidth&&y-c.ValidY<c.ValidHeight;};
        highPass(dev.Get(),root.Get(),pso.Get(),c,size.width,size.height,p,p,zero,zero);
        auto edited=[](UINT,UINT){return Pixel{.3f,.35f,.72f,1};};
        highPass(dev.Get(),root.Get(),pso.Get(),c,size.width,size.height,p,edited,zero,[&](UINT x,UINT y){return inside(x,y)?Pixel{.05f,-.05f,.02f,0}:zero(x,y);});
        c.Mode=1;c.TransferStrength=.75f;c.ColourStrength=.75f;
        highPass(dev.Get(),root.Get(),pso.Get(),c,28,23,p,p,zero,original);
    }
    auto negative=[](UINT x,UINT y){return Pixel{-.125f,.03f+float(x)*.01f,12.f+float(y)*.1f,.37f};};
    c.Mode=0;c.Passthrough=0;c.WhitePoint=2;c.Width=28;c.Height=23;
    for(UINT encoding:{0u,1u}){c.HighlightEncoding=encoding;highPass(dev.Get(),root.Get(),pso.Get(),c,28,23,negative,p,zero,negative,negative);}
    c.Mode=1;highPass(dev.Get(),root.Get(),pso.Get(),c,28,23,p,p,zero,negative,negative);
    // Varying HDR residual: compare against an independent CPU area integral of decoded pairs.
    c.Mode=6;
    auto varied=[](UINT x,UINT y){return Pixel{(x%2)?.7f:.1f,(y%2)?.8f:.2f,.5f,1};};
    auto changed=[&](UINT x,UINT y){auto v=varied(x,y);v[0]+=.025f;v[1]-=.03f;return v;};
    auto decode=[](double v){return v<.04045?v/12.92:pow((v+.055)/1.055,2.4);};
    const auto oracle=[&](UINT x,UINT y){
        if(x<c.ValidX||y<c.ValidY||x-c.ValidX>=c.ValidWidth||y-c.ValidY>=c.ValidHeight)return zero(x,y);
        double x0=double(x-c.ValidX)*32/c.ValidWidth,x1=double(x-c.ValidX+1)*32/c.ValidWidth;
        double y0=double(y-c.ValidY)*24/c.ValidHeight,y1=double(y-c.ValidY+1)*24/c.ValidHeight;Pixel result{};
        for(int j=int(floor(y0));j<int(ceil(y1));++j)for(int i=int(floor(x0));i<int(ceil(x1));++i){
            double weight=(std::min(x1,double(i+1))-std::max(x0,double(i)))*(std::min(y1,double(j+1))-std::max(y0,double(j)))/((x1-x0)*(y1-y0));
            auto a=varied(UINT(i),UINT(j)),b=changed(UINT(i),UINT(j));for(int ch=0;ch<3;++ch)result[ch]+=float((decode(b[ch])-decode(a[ch]))*weight);}
        return result;};
    highPass(dev.Get(),root.Get(),pso.Get(),c,32,24,varied,changed,zero,oracle);
    ...
    std::printf("PASS high-resolution deployed bytecode, debug=%d model_loaded=false\n",dbg);return 0;
