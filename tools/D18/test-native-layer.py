"""Build and run WARP pixel-reference checks for the DX11 native-layer compositor."""
import argparse
import json
import hashlib
import subprocess
...
fixture = Path(__file__).parent / 'tests/native-layer-host.cpp'
...
(out / 'validation.json').write_text(json.dumps(dict(passed=True, cases=4, api='WARP FL11.0/11.1', source_sha256=hashlib.sha256((source / 'dlssnr/Dx11NativeLayer.h').read_bytes()).hexdigest(), boundary='Synthetic pixel-reference cases; no gameplay acceptance'), indent=2), encoding='utf-8')
print(run.stdout, end='')
print('PASS: native-layer GPU pixel reference for FL11.0/11.1 and two UNORM formats')
