...
int wmain(int argc,wchar_t** argv)try{
    ...
    require(mode==L"shared_batch"||mode==L"shared_same_input"||mode==L"independent_batch","unsupported probe mode");
    const bool shared=mode!=L"independent_batch",sameInput=mode==L"shared_same_input";
    ...
    for(unsigned frame=0;frame<64;++frame){
        ...
        for(unsigned pass=0;pass<2;++pass){
            ...
            const int result=eval(cmd.Get(),features[instance],params[instance],sameInput?color.Get():previous,depth.Get(),shared&&pass>0?zeroMotion.Get():motion.Get(),answer,
