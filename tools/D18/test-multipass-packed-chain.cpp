...
void chain(ID3D12Device* device,ID3D12RootSignature* root,ID3D12PipelineState* pso,unsigned count,bool shared,bool custom,int failPass,DXGI_FORMAT motionFormat=DXGI_FORMAT_R32G32_FLOAT){
    ...
    auto upload=[&](bool filtered,D3D12_RESOURCE_STATES state,DXGI_FORMAT format=DXGI_FORMAT_R11G11B10_FLOAT){auto d=texture(width,height,true);d.Format=format;auto dst=create(device,d,D3D12_HEAP_TYPE_DEFAULT,D3D12_RESOURCE_STATE_COPY_DEST);
        ...
        for(UINT y=0;y<height;++y)for(UINT x=0;x<width;++x){Pixel v=filtered?Pixel{.25f,.5f,.75f,1}:Pixel{x%2?.75f:.125f,y%2?.5f:.25f,.75f,1};if(format==DXGI_FORMAT_R11G11B10_FLOAT){DirectX::PackedVector::XMFLOAT3PK packed(v[0],v[1],v[2]);memcpy(mem+y*fp.Footprint.RowPitch+x*4,&packed,4);}else{unsigned short half[4];for(int i=0;i<4;++i)half[i]=DirectX::PackedVector::XMConvertFloatToHalf(v[i]);memcpy(mem+y*fp.Footprint.RowPitch+x*8,half,8);}}
    ...
    auto* modelInput=upload(true,srv);g_nr.output=upload(true,srv);g_multi.input=upload(true,uav,DXGI_FORMAT_R16G16B16A16_FLOAT);g_multi.answer=upload(true,uav,DXGI_FORMAT_R16G16B16A16_FLOAT);
...
#include <shaders/dlssnr/Multipass_Execute_Dx12.inl>
    ...
    for(UINT y=0;y<height;++y)for(UINT x=0;x<width;++x){DirectX::PackedVector::XMFLOAT3PK expected(x%2?.75f:.125f,y%2?.5f:.25f,.75f);require(memcmp(mem+y*fp.Footprint.RowPitch+x*4,&expected,4)==0,"packed SR identity/failure must remain bit-exact");}
...
int wmain()try{
    ...
    for(unsigned count:{2u,3u,4u})for(bool shared:{false,true})for(bool custom:{false,true})chain(dev.Get(),root.Get(),pso.Get(),count,shared,custom,0);
    chain(dev.Get(),root.Get(),pso.Get(),2,true,true,0,DXGI_FORMAT_R16G16_FLOAT);
    for(bool shared:{false,true})chain(dev.Get(),root.Get(),pso.Get(),2,shared,true,2);
    ...
    puts("PASS packed HDR production orchestration 15 targeted cases, capture off, debug layer clean; identity/failure stub model, no game");return 0;
