...
int wmain(int argc,wchar_t** argv)try{
    require(argc==8,"forwarder runtime data-path driver-core mode output-directory style");
    const int style=_wtoi(argv[7]);require(style==0||style==2,"style 0 or 2");
    const std::wstring mode=argv[5];
    require(mode==L"shared_batch"||mode==L"anchored_shared"||mode==L"independent_batch","unsupported probe mode");
    const bool shared=mode!=L"independent_batch",anchored=mode==L"anchored_shared";
    ...
    NVSDK_NGX_Parameter* params[3]{};void* features[3]{};
    ...
    {HMODULE core=LoadLibraryExW(argv[4],nullptr,LOAD_LIBRARY_SEARCH_DLL_LOAD_DIR|LOAD_LIBRARY_SEARCH_DEFAULT_DIRS);require(core!=nullptr,"driver core load");
        ...
        for(unsigned i=0;i<3;++i){require(get(&params[i])==1&&params[i],"driver capability block");for(unsigned j=0;j<i;++j)require(params[i]!=params[j],"distinct capability blocks");}
    ...
    const float ratios[3]={1.f,1.f,1.f};
    for(unsigned i=0;i<3;++i){features[i]=createFeature(argv[2],argv[3],dev.Get(),cmd.Get(),params[i],w,h,0,1.f,style,1,1,-1,1,1,ratios[i]);
    ...
    auto firstReadback=buffer(dev.Get(),bytes,true),secondReadback=buffer(dev.Get(),bytes,true),thirdReadback=buffer(dev.Get(),bytes,true);
    ...
    for(unsigned frame=0;frame<64;++frame){
        ...
        for(unsigned pass=0;pass<3;++pass){
            const unsigned instance=anchored?(pass==0?0:1):shared?0:pass;auto* answer=pass==0?output.Get():pass==1?second.Get():third.Get();
            const bool reset=frame==0&&(!shared||pass==0||(anchored&&pass==1));
            const int result=eval(cmd.Get(),features[instance],params[instance],previous,depth.Get(),shared&&pass>(anchored?1u:0u)?zeroMotion.Get():motion.Get(),answer,
            ...
            copyReadback(answer,pass==0?firstReadback.Get():pass==1?secondReadback.Get():thirdReadback.Get(),D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
            previous=answer;
        ...
        barrier(cmd.Get(),third.Get(),D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE,D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
        submit();if(frame>=48){dump(firstReadback.Get(),L"pass1",frame);dump(secondReadback.Get(),L"pass2",frame);dump(thirdReadback.Get(),L"pass3",frame);}fflush(stdout);
    }
    memory("after_probe");
