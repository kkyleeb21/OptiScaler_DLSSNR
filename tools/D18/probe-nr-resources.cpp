...
int wmain(int argc,wchar_t** argv)try{
    ...
    require(mode==L"shared_batch"||mode==L"independent_batch","unsupported probe mode");
    const bool shared=mode==L"shared_batch";const unsigned featureCount=shared?1:2;
    const std::filesystem::path outputDirectory=argv[6];
    std::filesystem::create_directories(outputDirectory);
    ComPtr<ID3D12Debug> debug;const bool dbg=SUCCEEDED(D3D12GetDebugInterface(IID_PPV_ARGS(&debug)));if(dbg)debug->EnableDebugLayer();printf("debug_layer=%u style=%d warmup=16 measured=32\n",unsigned(dbg),style);
    ...
    ComPtr<ID3D12QueryHeap> timestamps;D3D12_QUERY_HEAP_DESC qd{};qd.Type=D3D12_QUERY_HEAP_TYPE_TIMESTAMP;qd.Count=4;check(dev->CreateQueryHeap(&qd,IID_PPV_ARGS(&timestamps)));
    auto timingReadback=buffer(dev.Get(),4*sizeof(UINT64),true);UINT64 frequency=0;check(queue->GetTimestampFrequency(&frequency));
    require(frequency>0,"timestamp frequency");printf("timestamp_frequency=%llu\n",frequency);
    ...
    require(admission.Budget>admission.CurrentUsage && admission.Budget-admission.CurrentUsage>6ull*1024*1024*1024,"resource probe needs 6 GiB headroom");
    ...
    {HMODULE core=LoadLibraryExW(argv[4],nullptr,LOAD_LIBRARY_SEARCH_DLL_LOAD_DIR|LOAD_LIBRARY_SEARCH_DEFAULT_DIRS);require(core!=nullptr,"driver core load");
        ...
        for(unsigned i=0;i<featureCount;++i){require(get(&params[i])==1&&params[i],"driver capability block");for(unsigned j=0;j<i;++j)require(params[i]!=params[j],"distinct capability blocks");}
    ...
    const float ratios[2]={ratio,ratio};
    for(unsigned i=0;i<featureCount;++i){features[i]=createFeature(argv[2],argv[3],dev.Get(),cmd.Get(),params[i],w,h,0,1.f,style,1,1,-1,1,1,ratios[i]);
    ...
    auto dump=[&](ID3D12Resource* rb,const wchar_t* stage,unsigned frame){
        ...
        D3D12_RANGE empty{};rb->Unmap(0,&empty);
    ...
    for(unsigned frame=0;frame<48;++frame){
        ...
        for(unsigned pass=0;pass<2;++pass){
            ...
            const bool reset=frame==0&&(!shared||pass==0);
            cmd->EndQuery(timestamps.Get(),D3D12_QUERY_TYPE_TIMESTAMP,2*pass);
            ...
            if(frame==47)copyReadback(answer,pass==0?firstReadback.Get():secondReadback.Get(),D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
            previous=answer;
    ...
    require(fclose(timingFile)==0,"timing close");
