...
int wmain(int argc,wchar_t** argv){
 ...
 const UINT tw=wcscmp(argv[3],L"4k")==0?3840u:128u,th=wcscmp(argv[3],L"4k")==0?2160u:128u;
 ...
 DXGI_SWAP_CHAIN_DESC sd{};sd.BufferDesc.Width=tw;sd.BufferDesc.Height=th;sd.BufferDesc.Format=DXGI_FORMAT_R8G8B8A8_UNORM;sd.SampleDesc.Count=1;sd.BufferUsage=DXGI_USAGE_RENDER_TARGET_OUTPUT;sd.BufferCount=1;sd.OutputWindow=w;sd.Windowed=TRUE;
 ...
 const char vsSource[]="struct O{float4 p:SV_Position;float2 uv:TEXCOORD0;}; O main(uint id:SV_VertexID){O o;o.uv=float2((id<<1)&2,id&2);o.p=float4(o.uv*float2(2,-2)+float2(-1,1),0,1);return o;}";
 ...
 for(UINT i=0;i<2;++i){D3D11_TEXTURE2D_DESC td{};td.Width=tw;td.Height=th;td.MipLevels=td.ArraySize=1;td.Format=DXGI_FORMAT_R10G10B10A2_UNORM;td.SampleDesc.Count=1;td.BindFlags=D3D11_BIND_RENDER_TARGET;
 ...
 ComPtr<ID3D11Predicate> predicate;D3D11_QUERY_DESC pred{D3D11_QUERY_OCCLUSION_PREDICATE,0};if(FAILED(d->CreatePredicate(&pred,&predicate)))return 13;c->Begin(predicate.Get());c->End(predicate.Get());c->SetPredication(nullptr,FALSE);
 ...
 for(unsigned frame=0;frame<90;++frame){
  ...
  if(range)c1->CSSetConstantBuffers1(1,2,oldCBs,oldFirst,oldCounts);else c->CSSetConstantBuffers(1,2,oldCBs);c->SetPredication(nullptr,FALSE);
  ...
  c->PSSetShader(ps.Get(),nullptr,0);if(predicated)c->SetPredication(predicate.Get(),TRUE);c->Draw(3,0);if(predicated)c->SetPredication(nullptr,FALSE);
 ...
 if(!predicated&&!missing){
  ...
  for(size_t i=0;i<finalPixels[0].size();++i)if((finalPixels[0][i]>>30)!=(finalPixels[1][i]>>30))return 34;
  printf("original_r=%.8f composed_r=%.8f alpha_preserved=1\n",mean[0],mean[1]);if(mean[0]<.20||mean[0]>.30||mean[1]<.10||mean[1]>.40)return 35;
 }
