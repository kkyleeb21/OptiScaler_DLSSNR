// Explicit owned-host INTERVENTION. One runtime CPU counter write per call; no code patches.
// Diagnostic only. Never deploy or attach this program to a game.
...
int wmain(int argc,wchar_t** argv){
    ...
    try{
        ...
        const bool coldStartOnly=std::wstring(argv[3])==L"counter-cold-start";const std::wstring phase=coldStartOnly?L"bindings":argv[3];need(phase==L"bindings","counter intervention requires bindings phase");need(argc==14&&std::wstring(argv[9])==L"feedback_live_shared"&&std::wstring(argv[11])==L"2"&&std::wstring(argv[13])==L"initial","fixed C14 two-pass shared style2 initial-reset host only");need(phase==L"controls"||phase==L"state"||phase==L"bindings"||phase==L"resources","observer phase");const bool statePhase=phase==L"state",bindingsPhase=phase==L"bindings",resourcesPhase=phase==L"resources";std::vector<ULONG64> states;
        ...
        log<<"{\"schema\":\"d18-nr-counter-intervention-v1\",\"event\":\"start\",\"pid\":"<<pi.dwProcessId<<",\"counter_policy\":\""<<(coldStartOnly?"second_cold_start":"per_pass")<<"\""<<",\"thread\":"<<pi.dwThreadId<<",\"resources_phase\":"<<int(resourcesPhase)<<",\"bindings_phase\":"<<int(bindingsPhase)<<",\"state_phase\":"<<int(statePhase)<<",\"code_modified\":false,\"data_modified\":true,\"intervention\":\"owned-host counter isolation; see counter_policy and per-hit values\",\"coverage\":\"owned host main thread only\",\"max_hits\":768}\n";
        while(!exited){
                    ...
                    if(point<0)status=DBG_EXCEPTION_NOT_HANDLED;
                        ...
                        if(point==0){
                            ...
                            if(point==1){++gateCalls;need(gateCalls==controlCalls,"consumer entry pairing");
                                const auto state=read<ULONG64>(pi.hProcess,c.Rcx+0x178);
                                const auto before=read<ULONG64>(pi.hProcess,state+0xc8),banked=coldStartOnly?(controlCalls==2?0:before):ULONG64((controlCalls-1)/2);SIZE_T written=0;
                                need(WriteProcessMemory(pi.hProcess,reinterpret_cast<void*>(state+0xc8),&banked,sizeof(banked),&written)&&written==sizeof(banked),"owned-host counter write");
                                need(read<ULONG64>(pi.hProcess,state+0xc8)==banked,"counter write readback");
                                event<<",\"counter_before_intervention\":"<<before<<",\"counter_after_intervention\":"<<banked;
                                event<<",\"layer\":"<<c.Rcx<<",\"state\":"<<state<<",\"counter\":"<<read<ULONG64>(pi.hProcess,state+0xc8)<<",\"return_address\":"<<read<ULONG64>(pi.hProcess,c.Rsp);
        ...
        CloseHandle(pi.hThread);CloseHandle(pi.hProcess);puts("PASS owned-host COUNTER INTERVENTION; data changed, debugger perturbs scheduling");return 0;
